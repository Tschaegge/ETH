...
public class EnvironmentGenerator {
	...
	public EnvironmentGenerator(SootMethod method, PointsToInitializer pointsTo) {
		...
		// TODO: FILL THIS OUT (Done not exactly sure why one needs pointsTo)
		//Extracts all the local variable names from the Soot method and creates an Apron environment from it 
		for(Local l:method.getActiveBody().getLocals()){
			logger.debug("local "+l.getName());
			ints.add(l.getName());
