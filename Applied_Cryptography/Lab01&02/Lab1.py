from Crypto.Hash import SHA256
import hashlib; 
print(hashlib.sha224(b"you made it").hexdigest())
print(SHA256.new(data=b'hi').hexdigest())
text = b'.'
print(len(text))
sol = [text[i:i+16] for i in range(0, len(text), 16)]
print(sol)
res = b''
for el in sol:
    res += bytes([el[-1]])
print(res)
print(SHA256.new(data=res).hexdigest())

print(bytes.fromhex("596f752063616e206465636f646521"))

print("ጷ뼯쯾".encode("utf-8").hex())

crypt = bytes.fromhex("210e09060b0b1e4b4714080a02080902470b0213470a0247081213470801470a1e4704060002")
print(crypt)


word = ""
for j in range(0, len(crypt)):
    res = bytes([byte ^ 103 for byte in crypt])
    word = res.decode("utf-8", errors='replace')
print(103, word)


def otp(message,key):
    """message and key need to have the same lenght"""
    return (bytes(x ^ y for (x, y) in zip(message, key))).hex()


message = "Pay no mind to the distant thunder, Beauty fills his head with wonder, boy"
key = bytes.fromhex("bca914890bc40728b3cf7d6b5298292d369745a2592ad06ffac1f03f04b671538fdbcff6bd9fe1f086863851d2a31a69743b0452fd87a993f489f3454bbe1cab4510ccb979013277a7bf")
message = message.encode("utf-8")
print(otp(message,key))

text1 = '9b51325d75a7701a3d7060af62086776d66a91f46ec8d426c04483d48e187d9005a4919a6d58a68514a075769c97093e29523ba0'
text2 = 'b253361a7a81731a3d7468a627416437c22f8ae12bdbc538df0193c581142f864ce793806900a6911daf213190d6106c21537ce8760265dd83e4'

def xor(X, Y):
    return bytes(x ^ y for (x, y) in zip(X, Y))
text1 = bytes.fromhex(text1)
text2 = bytes.fromhex(text2)
sol = ""
key = xor(text2,"flag{".encode())
for i in range(0,len(text1),5):
    res = xor(text1[i:i+5],key)
    sol += res.decode("utf-8")
    key= xor(res,text2[i+5:i+10])
print(sol)    

CHALLENGE_CIPHERTEXT = ""
separated = [CHALLENGE_CIPHERTEXT[i:i+120] for i in range(0, len(CHALLENGE_CIPHERTEXT), 120)]
separated_bytes = [bytes.fromhex(hex_str) for hex_str in separated]

